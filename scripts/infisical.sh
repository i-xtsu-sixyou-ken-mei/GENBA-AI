#!/usr/bin/env bash
# Run a command with secrets from one of the two Infisical projects GENBA uses.
#
#   scripts/infisical.sh genba -- <cmd...>
#     GENBA AI project (repo default, .infisical.json). GENBA's own
...
#     GENBA only reads SUPABASE_URL from it. NEVER write to it
...
GENBA_PROJECT_ID="c9422b82-48af-4941-9660-fca70a351259"
ZAP_PROJECT_ID="572f3b7e-dd26-4f40-9695-df1a3d2d951b"

usage() {
  echo "usage: $0 <genba|zap> -- <command...>" >&2
  exit 64
}

[[ $# -ge 3 && $2 == "--" ]] || usage
case "$1" in
  genba) project_id=$GENBA_PROJECT_ID ;;
