-- Expose schema `kokode_ai` to PostgREST so KOKODE's `genba-lead` Edge Function can insert
-- via the Data API. Idempotent; apply with:
--   npm run ops -- sql supabase/migrations/20260923021804_expose_kokode_ai_schema.sql
...
-- `kokode_ai` instead of appending it.
